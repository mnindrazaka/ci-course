...
@section('content')
    <form action="{{ base_url('blog/update/' . $blog->id) }}" method="post" enctype="multipart/form-data">
        <div class="form-group">
            <label for="judul">Judul</label>
            <input type="text" name="judul" id="judul" class="form-control" placeholder="Masukkan Judul" value="{{ old('judul', $blog->judul) }}">
            @if($errors->has('judul'))
        ...
        <div class="form-group">
            <textarea class="form-control" id="summernote" name="isi" rows="3" title="isi">{{ old('isi', $blog->isi) }}</textarea>
            @if($errors->has('isi'))
